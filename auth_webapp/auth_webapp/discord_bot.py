...
def link_discord(request):
    ...
    try:
        token = jwt.decode(request.POST['token'], settings.JWT_SECRET1, algorithms='HS256', options={
                           "require": ["discord_id", "auth_token"]})
        user_token = jwt.decode(token['auth_token'], settings.JWT_SECRET2,
                                algorithms='HS256', options={"require": ["buck_id"]})

        print(f"Processing token for {token['discord_id']}")
        try:
            discord_linked = OSUUser.objects.get(
        ...
        if user.discord_id is None:
            ...
            user.save()
            print(
                f"Successfully linked {user.discord_id} to shib {user.shib_id}")
            return JsonResponse({"success": True, "affiliation": user.affiliation})
        else:
            print(
                f"User {user.shib_id} is already linked to another discord user")
