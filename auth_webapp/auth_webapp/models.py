...
class OSUUser(AbstractBaseUser, PermissionsMixin):
    ...
    discord_id = models.CharField(max_length=100, null=True, blank=True) # You can change this type if needed
    ...
    affiliation = models.CharField(max_length=20, choices=aff_choices.choices, default=aff_choices.NONE)
    ...
    def is_currently_on_mailing_list(self):
        ...
        try:
            list_info = client.lists.get_list_member(os.environ['MAILCHIMP_LIST_ID'], hashlib.md5(email.encode()).hexdigest())
    ...
    def add_to_mailing_list(self):
        email = (self.name_num+"@osu.edu").lower()
        try:
            list_info = client.lists.add_list_member(os.environ['MAILCHIMP_LIST_ID'], {'email_address': email, 'status': 'subscribed'})
            if list_info['status'] == 'subscribed':
                self.added_to_mailing_list = True
                self.save()
                return True
