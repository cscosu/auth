...
def new_member(request):
    ...
    if request.user.is_authenticated:
        discord_token = jwt.encode({"buck_id": request.user.shib_id, "date": str(datetime.date.today())}, settings.JWT_SECRET2, algorithm='HS256')
...
def login_view(request):
    ...
    user, created = OSUUser.objects.get_or_create(shib_id=shib_id, defaults={"display_name": request.headers['Displayname'], "name_num": name_n})
...
def debug_login(request):
    ...
    # Lets create a user
    user, created = OSUUser.objects.get_or_create(shib_id=request.GET['id'], defaults={"display_name": "test user %s" % request.GET['id'], "name_num": "test.%s" % request.GET['id']})
...
def _user_can_submit_attendance(user):
    try:
        last_attend = AttendanceRecord.objects.filter(user=user).order_by('-date_recorded')[0]
...
def attend(request):
    if _user_can_submit_attendance(request.user):
        ar = AttendanceRecord(user=request.user, date_recorded=datetime.datetime.now().astimezone(ohio_tz))
        ar.save()
...
def subscribe(request):
    ...
    if request.user.add_to_mailing_list():
        messages.add_message(request, messages.INFO, 'You have been added to our mailing list!')
    else:
        messages.add_message(request, messages.ERROR, 'Sorry, something went wrong adding you to the mailing list. Try http://mailinglist.osucyber.club instead, and let us know.')
